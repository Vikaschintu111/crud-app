...
const CrudOperations = () => {
  ...
  // Create a new team
  const handleCreate = () => {
    axios.post(apiUrl, newTeam)
      .then(response => {
        setTeams([...teams, response.data]);
        setNewTeam({ name: '', role: '', age: '', matches: '' , runs: '', wickets: ''});
      })
  ...
  const handleUpdate = (id) => {
    axios.put(`${apiUrl}/${id}`, newTeam)
      .then(response => {
        ...
        setNewTeam({ name: '', role: '', age: '', matches: '', runs: '',wickets: '' }); // Reset form
      })
  ...
  return (
    <div>
      ...
      <h2>{selectedTeam ? 'Edit Team' : 'Create New Team'}</h2>
        ...
        />
        {selectedTeam ? (
          <button onClick={() => handleUpdate(selectedTeam._id)}>Update Team</button>
        ) : (
